...
from app.schemas.candidate import CandidateExportRequest, SwipeFeedbackRequest
...
def get_candidates(
    jobId: str = Query(...),
    mode: str = Query("volume", pattern="^(volume|elite)$"),
    refresh: bool = Query(False),
    _: dict = Depends(get_current_user),
    db: Session = Depends(get_db),
):
    candidates = fetch_ranked_candidates(db=db, job_id=jobId, mode=mode, refresh=refresh)
